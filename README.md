...
## RSK (RootStock) is compatible with the EVM (Ethereum Virtual Machine) 
...
For Basic of Bitcoin https://github.com/bitcoinbook/bitcoinbook
For Basic of RSK https://www.rsk.co/Whitepapers/RSK-White-Paper-Updated.pdf
</p>
...
Pete Scandlon of Pete's Pet Shop is interested in using RSK as an efficient way to handle their pet adoptions. The store has space for 16 pets at a given time, and they already have a database of pets. As an initial proof of concept, **Pete wants to see a dapp which associates an blockchain address with a pet to be adopted.**
...
We also will be using [Ganache](/ganache), a personal blockchain for Ethereum and blockchain development you can use to deploy contracts, develop applications, and run tests. You can download Ganache by navigating to http://truffleframework.com/ganache and clicking the "Download" button.
...
Solidity is a statically-typed language, meaning data types like strings, integers, and arrays must be defined. **Solidity has a unique type called an address**. Addresses are Ethereum/RSK addresses, stored as 20 byte values. Every account and smart contract on the Ethereum/RSK blockchain has an address and can send and receive Ether to and from this address.
...
* We've defined a single variable: `adopters`. This is an **array** of Ethereum/RSK addresses. Arrays contain one type and can have a fixed or variable length. In this case the type is `address` and the length is `16`.
...
Solidity is a compiled language, meaning we need to compile our Solidity to bytecode for the RSK Virtual Machine (EVM) to execute. Think of it as translating our human-readable Solidity into something the EVM understands.
...
1. Before we can migrate our contract to the blockchain, we need to have a blockchain running. For this tutorial, we're going to use [Ganache](/ganache), a personal blockchain for blockchain development you can use to deploy contracts, develop applications, and run tests. If you haven't already, [download Ganache](/ganache) and double click the icon to launch the application. This will generate a blockchain running locally on port 7545.
...
1. Examine the file. Note that there is a global `App` object to manage our application, load in the pet data in `init()` and then call the function `initWeb3()`. The [web3 JavaScript library](https://github.com/ethereum/web3.js/) interacts with the RSK blockchain. It can retrieve user accounts, send transactions, interact with smart contracts, and more.
...
* First, we check if we are using modern dapp browsers or the more recent versions of [MetaMask](https://github.com/MetaMask) or [Nifty wallet](https://github.com/poanetwork/nifty-wallet) where an `RSK TESTNET` provider is injected into the `window` object. If so, we use it to create our web3 object /* , but we also need to explicitly request access to the accounts with `ethereum.enable()` */.
...
1. In the box marked **Wallet Seed**, enter the mnemonic that is displayed in Ganache.
   ...
   **Warning**: Do not use this mnemonic on the main RSK network (mainnet). If you send ETH to any account generated from this mnemonic, you will lose it all!
   </p>
...
1. You'll see the button next to the adopted pet change to say "Success" and become disabled, just as we specified, because the pet has now been adopted.
   ...
   And in MetaMask/Nifty, you'll see the transaction listed:
